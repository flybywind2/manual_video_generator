...
- [ ] 변경 전 `docs/workflow-codebase-structure.md`에서 해당 단계와 계약을 확인한다.
- [ ] workflow step 문자열, artifact 이름, degraded reason은 테스트 없이 추가하지 않는다.
- [ ] 사용자가 보는 UI 변화는 `test_home_ui.py` 또는 browser smoke로 검증한다.
- [ ] pipeline/package 변화는 `tools/verify_package.py`와 관련 pytest로 검증한다.
- [ ] 운영계 자동조작에 가까운 변경은 `RiskPolicy`, `ApprovalGate`, audit log 영향을 먼저 확인한다.

## P0 - Workflow Correctness

- [ ] `WorkflowStep`/`PipelineStage` 상수 모듈 도입
...
- [ ] 로그인 이후 deferred MCP live rehearsal을 명시적 단계로 승격
...
- [ ] 직접 시연 replay 실패 시 사용자 판단 가능한 산출물/상태 강화
...
- [ ] AI 자동 실행에서 로그인 화면 감지 후 멈춤/요청 정책 정리
...
- [ ] `pipeline.py`에서 BrowserRunner 모듈 분리
...
- [ ] `doctor.ps1` 결과와 `/api/config/status` 항목 정렬
...
- [ ] smoke 산출물 cleanup/ignore 정책 정리
...
- [ ] degraded reason을 사용자 친화 문구로 변환
...
- [ ] property-based redaction test 검토
...
- [ ] Browser Agent 실제 화면 smoke fixture 확장
